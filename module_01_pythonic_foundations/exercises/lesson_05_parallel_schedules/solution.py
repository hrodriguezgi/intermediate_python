def build_lesson_schedule(
    ...
    return [
        ...
        for number, (topic, minutes, instructor) in enumerate(
            zip(topics, durations, instructors), start=1
        )
    ]
...
if __name__ == "__main__":
    ...
    for lesson in schedule:
        print(f"  Lesson {lesson['number']}: {lesson['topic']} "
              f"({lesson['minutes']} min, instructor: {lesson['instructor']})")
